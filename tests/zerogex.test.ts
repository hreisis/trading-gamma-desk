...
describe('ZeroGEX reference',()=>{
 ...
 it('does not write an unchanged provider snapshot again',async()=>{const target=store();const fetcher=vi.fn().mockResolvedValue(new Response(JSON.stringify({result:{structuredContent:data}})));await loadZeroGex('SPY',target,fetcher);expect(target.writeText).not.toHaveBeenCalled();});
 it('writes only when the provider snapshot advances',async()=>{const target=store();const fetcher=vi.fn().mockResolvedValue(new Response(JSON.stringify({result:{structuredContent:{...data,as_of:'2026-09-15T03:16:00+00:00'}}})));await loadZeroGex('SPY',target,fetcher);expect(target.writeText).toHaveBeenCalledOnce();});
