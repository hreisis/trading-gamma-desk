...
export function OverviewCards({view, lang, introduction}: {view: V2CommandCenterPageView; lang: V2Language; introduction?: ReactNode}) {
 ...
 return <div className={styles.overview}>
  ...
  <div className={styles.grid}>
   <article className={styles.card}><h3>{t('STRUCTURAL RISK','结构风险')}</h3><svg viewBox="0 0 200 145" role="img" aria-label={`${t('Structural risk','结构风险')} ${num(risk)} / 100`} className={styles.gauge}>
   ...
   <article className={styles.card}><h3>{t('TACTICAL OPPORTUNITY','回撤机会')}</h3><strong className={styles.big}>{num(view.opportunityScore)}<small> / 100</small></strong><p>{t('Potential dislocation, not a buy trigger','潜在回撤机会，不是买入触发器')}</p><div className={styles.track}><i style={{width:`${view.opportunityScore??0}%`}}/></div><b>{t('Confirmation','修复确认')}: {view.opportunity?.confirmation==='broadening'?t('Broadening','参与扩大'):view.opportunity?.confirmation==='recovering'?t('Recovery starting','修复启动'):view.opportunity?.confirmation==='unconfirmed'?t('Unconfirmed','尚未确认'):t('Unavailable','暂无数据')}</b>{view.opportunity?.eventBlocked&&<small>{t('Event conditions require review','事件条件待评估')}</small>}</article>
