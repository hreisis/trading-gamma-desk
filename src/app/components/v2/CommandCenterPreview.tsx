...
function riskLabel(score: number | null) {
  ...
  if (score <= 40) return "Low / Supportive";
  if (score <= 65) return "Neutral / Moderate";
  return "Elevated / Defensive";
}
...
function Ticker({ view, symbol }: { view: V2CommandCenterPageView; symbol: string }) {
  ...
  return (
    <span className="pv-ticker">
      <b>{symbol}</b>
      <strong>{quote?.latestPrice == null ? "—" : fmt(quote.latestPrice, quote.latestPrice >= 100 ? 2 : 2)}</strong>
...
function structureRegimeLabel(view: V2CommandCenterPageView, symbol: "SPY" | "QQQ") {
  ...
  if (cone?.interpretation.regime) return cone.interpretation.regime.replaceAll("_", " ").toUpperCase();
  if (g?.dealerFlowRegime?.toLowerCase().includes("stabilizing")) return "STABILIZING";
  return (g?.regime ?? "STRUCTURE").replaceAll("_", " ").toUpperCase();
}
    ...
    <article className="pv-structure-card">
      <div className="pv-symbol-head">
        <div>
          ...
          <span>{symbol === "SPY" ? "SPDR S&P 500 ETF" : "Invesco QQQ Trust"}</span>
        </div>
        <div className="pv-regime">{structureRegimeLabel(view, symbol)}</div>
      </div>
      <div className="pv-price">{fmt(g.spot, 2)}</div>
...
function RotationSection({ view }: { view: V2CommandCenterPageView }) {
  ...
  return (
    <section className="pv-flat pv-rotation">
      <h3>SECTOR ROTATION <small>(vs SPY)</small></h3>
...
function TechnologySection({ view }: { view: V2CommandCenterPageView }) {
  ...
  return (
    <section className="pv-flat pv-tech">
      <h3>TECHNOLOGY INTERNAL <small>(vs XLK)</small></h3>
      {rows.length === 0 ? (
    ...
    <aside className="pv-right">
      <section className="pv-ai" id="ai-study">
        ...
        <div className="pv-confidence">
          <div><strong>CONFIDENCE</strong><span>{view.aiStudy.confidence} ({conf}/100)</span></div>
          <i><b style={{ width: `${conf}%` }} /></i>
        </div>
      </section>
      ...
      <section className="pv-review" id="daily-review">
        <h2>DAILY REVIEW</h2>
        <div><strong>Morning stance</strong><span>{view.dailyReview.morningStance ?? view.stance ?? "—"}</span></div>
        <div><strong>Actual outcome</strong><span>{view.dailyReview.actualOutcome || "Pending"}</span></div>
        <div><strong>What worked</strong><span>{view.dailyReview.whatWorked[0] ?? "Pending"}</span></div>
        <div><strong>Tomorrow watch</strong><span>{view.dailyReview.tomorrowWatch[0] ?? "Pending"}</span></div>
      </section>
    </aside>
  );
}
  ...
  return (
    <div className="pv-app">
      ...
      <aside className="pv-nav">
        ...
        <a className="pv-settings">⚙ <span>Settings</span></a>
      </aside>
      ...
      <div className="pv-shell">
        <header className="pv-top">
          ...
          <div className="pv-open">● Market Open</div>
        </header>
        ...
        <main className="pv-main">
          <div className="pv-center">
            <section id="overview">
              ...
              <div className="pv-overview">
                <article>
                  ...
                  <p>{view.macroSummary?.interpretation ?? view.macroLabel ?? "Markets direction, not price, confirms the next move."}</p>
                </article>

                <article>
                  <h3>SENTIMENT / RISK</h3>
                  <div className="pv-gauge"><div className="arc" /><b>{view.riskScore ?? "—"}</b></div>
                  <p className="center">{riskLabel(view.riskScore)}</p>
                </article>
            ...
            <section id="structure" className="pv-structure">
              ...
              <div className="pv-structure-cards"><StructureCard view={view} symbol="SPY" /><StructureCard view={view} symbol="QQQ" /></div>
            </section>
