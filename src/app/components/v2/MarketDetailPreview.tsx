...
const EN = {
  ...
  exposure: "EXPOSURE",
  overallExposure: "Overall Market Exposure",
...
function RiskSnapshot({ view, copy }: { view: V2CommandCenterPageView; copy: MarketCopy }) {
  ...
  const action = highBetaAction(view);
  const actionTone = action === "ADD" ? "good" : action === "TRIM" ? "bad" : "neutral";
  return (
    <section className="mk-card mk-risk-snapshot">
      ...
      <div className="mk-snapshot-body">
        ...
        <small>{copy.highBeta}: <em className={actionTone}>{action}</em>{highBeta == null ? "" : ` · ${copy.target} ${highBeta}%`}</small>
      </div>
    </section>
  );
}
...
export function MarketDetailPreview({
  ...
  const exposurePointer = exposureMid == null
    ? 50
    : Math.max(0, Math.min(100, (Math.max(-100, Math.min(100, exposureMid)) + 100) / 2));
  const factorOrder = [
        ...
        <main className="mk-main">
          <div className="mk-center">
            <section className="mk-overview-row">
              ...
              <article className="mk-card mk-exposure"><h2>{copy.exposure}</h2><p>{copy.overallExposure}</p><strong>{exposureMid == null ? "—" : `${exposureMid}%`}</strong><small>{exposureMid == null ? copy.unavailable : exposureMid === 0 ? copy.neutral : copy.recommended}</small><div className="mk-exposure-bar"><i/><b style={{ left: `${exposurePointer}%` }}/></div><div className="mk-exposure-labels"><span>-100%</span><span>0%</span><span>+100%</span></div></article>
            </section>
