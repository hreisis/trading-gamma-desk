...
Home display cache v1: successful live views are saved per language after response. Nested Suspense serves the last successful complete view while the same request refreshes providers and scores. Banner states saved time and updating status; maximum display-cache age 24h. Demo, source overrides and Gamma fixtures neither read nor write it. No changes to provider refresh cadence or score calculations. Initial requests without cache retain a lightweight shell.
