...
| M2-5B | Unified incremental catalyst update | `npm run catalyst:update` (`--dry-run` / `--max-events 2` / `--force`); stages official_facts→AI brief ∥ 4A→4B→4C; identity incremental skip; run lock + stale recovery; atomic manifest `update-latest.json`; branch isolation; no scheduler — ✅ |
