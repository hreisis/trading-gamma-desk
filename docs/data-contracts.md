...
Home display cache v1: successful live views are saved per language after response. Nested Suspense serves the last successful complete view while the same request refreshes providers and scores. Banner states saved time and updating status; maximum display-cache age 24h. Demo, source overrides and Gamma fixtures neither read nor write it. No changes to provider refresh cadence or score calculations. Initial requests without cache retain a lightweight shell. Display-cache writes are request-triggered and capped at one per language per 15-minute window. ZeroGEX remains request-triggered and only persists a strictly newer provider snapshot.

Cleanup: removed unused light-preview components, unused risk-score display, legacy gauge DOM observer and the route-level loading screen. The no-cache homepage renders normal navigation/section structure; cached and completed views are unchanged. Provider, score and model logic remain unchanged. Persistence tests now clean only their own temporary directories. Retained legacy helpers still covered by tests and operational scripts.
