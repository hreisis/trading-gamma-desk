...
| `catalyst/` | Raw event → canonical Catalyst; dedupe; feed query; calendar + BLS results + documents + rule-based briefs + optional AI briefs | Normalize/extract are pure; fetch/build/enhance scripts are local-only and independent; public demo never networks / never calls LLM |
