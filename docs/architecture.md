...
| `catalyst/` | Raw event → canonical Catalyst; dedupe; feed query; calendar + BLS results + documents + rule-based briefs | Normalize/extract are pure; fetch/build scripts are local-only and independent; public demo never networks |
