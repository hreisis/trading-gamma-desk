...
| `catalyst/` | Raw event → canonical Catalyst; dedupe; feed query; calendar + BLS results + documents + rule-based briefs + optional AI briefs + market-context ETF snapshots + deterministic reaction patterns + optional AI market-reaction narratives + optional integration smoke | Normalize/extract/compute/classify are pure; fetch/build/enhance/smoke scripts are local-only and independent; public demo never networks / never calls LLM or Alpaca; integration smoke requires explicit `--live` |
