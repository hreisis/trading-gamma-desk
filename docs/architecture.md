...
| `catalyst/` | Raw event → canonical Catalyst; dedupe; feed query; official calendar adapters | Normalize is pure; `catalyst:fetch` pulls BLS/BEA/FOMC **schedules** locally only; public demo never networks |
