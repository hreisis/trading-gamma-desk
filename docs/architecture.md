...
| `studies/` | PIT archive (M5-1B) + study definitions & forward outcomes (M5-2) + similar-regime study (M5-3) | `DailyResearchArchive` from fixtures; `StudyDefinition`/`StudyForwardOutcome` with trading-session returns; `StudyMatchProfile`/`SimilarRegimeStudy` with exact PIT matching; outcomes never in replay inputs or match selection |
