...
| `studies/` | PIT archive (M5-1B) + study definitions & forward outcomes (M5-2) + similar-regime study (M5-3) + evidence bundle (M5-4) | `DailyResearchArchive` from fixtures; `StudyDefinition`/`StudyForwardOutcome`; `SimilarRegimeStudy` + `StudyEvidenceBundle` with explicit PIT matching and status rules; outcomes never in replay inputs or match selection |
| `study-agent/` | Constrained LLM study memo (M6-1) | `StudyMemo` from `StudyEvidenceBundle` only; OpenAI structured outputs + local citation/guardrail validation; abstain without LLM when evidence insufficient |
