...
| `studies/` | PIT archive (M5-1B) + study definitions & forward outcomes (M5-2) | `DailyResearchArchive` from fixtures; `StudyDefinition`/`StudyForwardOutcome` with trading-session returns; outcomes never in replay inputs |
